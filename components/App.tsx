...
export const App = () => {
  ...
  const Convert = async (record: DataType) => {
    let currentEURate: any = await apiGet(`/api/rate?rate=${record.currency}`);
  ...
  const handleAmountChange = (e) => {
    ...
    setAmount(amount);
    baseCurrencyRate && setTotal((amount * baseCurrencyRate.rate).toFixed(2));
  };
  ...
  return (
    <>
      ...
      <Wrapper>
        ...
        />
        <Button disabled={values.length < 1} onClick={handleAdd} type="primary">
          Add
        </Button>
      </Wrapper>
      <Table
